...
(use-package vc
  ...
  (with-eval-after-load 'vc-dir
    ...
    (defun emacs-kit/vc-git-command (verb fn)
    ...
    (defun emacs-kit/vc-git-add (&optional _revision _vc-fileset _comment)
      (interactive "P")
      (emacs-kit/vc-git-command "Staged" 'vc-git-register))

    (defun emacs-kit/vc-git-reset (&optional _revision _vc-fileset _comment)
      (interactive "P")
      (emacs-kit/vc-git-command "Unstaged"
                                 (lambda (files) (vc-git-command nil 0 files "reset" "-q" "--")))))


  (defun emacs-kit/vc-git-visualize-status ()
  ...
  (defun emacs-kit/vc-pull-merge-current-branch ()
  ...
  (defun emacs-kit/vc-browse-remote (&optional current-line)
...
Otherwise, open the repository's main page."
  ...
  (defun emacs-kit/vc-diff-on-current-hunk ()
  ...
  (defun emacs-kit/switch-git-status-buffer ()
...
The completion candidates include the Git status of each file."
  ...
  ;; For *vc-dir* buffer:
  (with-eval-after-load 'vc-dir
    (define-key vc-dir-mode-map (kbd "S") #'emacs-kit/vc-git-add)
    (define-key vc-dir-mode-map (kbd "U") #'emacs-kit/vc-git-reset)
    (define-key vc-dir-mode-map (kbd "V") #'emacs-kit/vc-git-visualize-status)
  ...
  ;; For C-x v ... bindings:
  (define-key vc-prefix-map (kbd "S") #'emacs-kit/vc-git-add)
  (define-key vc-prefix-map (kbd "U") #'emacs-kit/vc-git-reset)
  (define-key vc-prefix-map (kbd "V") #'emacs-kit/vc-git-visualize-status)
  (define-key vc-prefix-map (kbd "R") #'emacs-kit/vc-git-reflog)
  (define-key vc-prefix-map (kbd "B") #'emacs-kit/vc-browse-remote)
  (define-key vc-prefix-map (kbd "o") #'(lambda () (interactive) (emacs-kit/vc-browse-remote 1)))
  (define-key vc-prefix-map (kbd "=") #'emacs-kit/vc-diff-on-current-hunk)

  ;; Switch-buffer between modified files
  (global-set-key (kbd "C-x C-g") 'emacs-kit/switch-git-status-buffer))
